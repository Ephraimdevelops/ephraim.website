...
export function ProjectModal({ project: initialProject, onClose }: ProjectModalProps) {
    if (!initialProject) return null;

    return (
        <ModalContent project={initialProject} onClose={onClose} />
    );
}

function ModalContent({ project: initialProject, onClose }: { project: Project, onClose: () => void }) {
    // Fetch full details including all image URLs
    const projectDetails = useQuery(api.projects.getById, { id: initialProject._id });

    // Use fetched data if available, otherwise fall back to initial prop (which might differ slightly if outdated)
    const project = projectDetails || initialProject;

    // We can assume if projectDetails is loaded, it has `imagesUrls` (if we typed it correctly or cast it)
    const galleryUrls = (project as any).imagesUrls as string[] | undefined;
        ...
        <AnimatePresence>
            ...
            >
                ...
                <div className="container-narrow py-20 md:py-32">
                    ...
                    {/* Main Image */}
                    {/* Prefer coverImageUrl, fall back to constructing if necessary (though we avoid constructing now) */}
                    {(project.coverImageUrl || project.coverImage) && (
                        <motion.div
                        ...
                        >
                            {project.coverImageUrl ? (
                                ...
                                // Fallback or loading state if URL not yet resolved (shouldn't happen if getById is fast, 
                                // or if initialProject had it. If initialProject was from list, it has coverImageUrl)
